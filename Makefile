...
settings.hex: nrf52840_xxaa
	nrfutil settings generate \
		...
		--key-file "$(SDK_ROOT)/examples/dfu/secure_bootloader/private.pem" \
		"$(OUTPUT_DIRECTORY)/settings.hex"

firmware.hex: settings.hex
	mergehex --merge \
		"$(SDK_ROOT)/components/softdevice/s140/hex/s140_nrf52_7.2.0_softdevice.hex" \
		"$(SDK_ROOT)/examples/dfu/secure_bootloader/pca10056_s140_ble/armgcc/_build/nrf52840_xxaa_s140.hex" \
		"$(OUTPUT_DIRECTORY)/nrf52840_xxaa.hex" \
		"$(OUTPUT_DIRECTORY)/settings.hex" \
		--output "$(OUTPUT_DIRECTORY)/firmware.hex"

flash: firmware.hex
	"$(GNU_INSTALL_ROOT)/arm-none-eabi-gdb" -nx --batch \
